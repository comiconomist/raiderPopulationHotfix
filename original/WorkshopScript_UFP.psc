...
;-----------------------------------------------------------
;	UFO4P 1.0.3 Bug #20581 - GENERAL NOTE
;-----------------------------------------------------------

;Throughout this script, calls of the CreateActor, ModifyResourceData and SetResourceData functions on WorkshopParentScript have been replaced with
;calls of public versions of those functions (CreateActorPUBLIC, ModifyResourceDataPUBLIC and SetResourceDataPUBLIC; for this purpose, the already
;existing CreateActorPUBLIC function on WorkshopParentScript has been modified to comply with its non-public version, while ModifyResourceDataPUBLIC
;and SetResourceDataPUBLIC have been newly created). The public versions of the functions wait until the edit lock on WorkshopParentScript is released
;and only then call the non-public versions (i.e. the actual functions). This is to make sure that there will be no interfering threads running on
;WorkshopParentScript that could potentially try to manipulate sensitive data at the same time. While the chances for this to happen are low for these
;three functions, potential issues could not be safely excluded. The modifications will safely prevent this from happening though.
;
;All other edits are indicated (and commented) individually.
...
Event OnTimer(int aiTimerID)
	...
	elseIf aiTimerID == UFO4P_DailyUpdateResetHappinessTimerID
		DailyUpdate(bRealUpdate = false, bResetHappiness = true)
	endif
EndEvent
...
function SetOwnedByPlayer(bool bIsOwned)
	...
	elseif bIsOwned && !OwnedByPlayer
		...
		if currentHappiness < minHappinessClearWarningThreshold || currentHappinessTarget < minHappinessClearWarningThreshold
			;UFO4P 1.0.3 Bug #20581
			WorkshopParent.ModifyResourceDataPUBLIC(WorkshopParent.WorkshopRatings[WorkshopParent.WorkshopRatingHappiness].resourceValue, self, minHappinessClearWarningThreshold)		
			WorkshopParent.ModifyResourceDataPUBLIC(WorkshopParent.WorkshopRatings[WorkshopParent.WorkshopRatingHappinessTarget].resourceValue, self, minHappinessClearWarningThreshold)		
		EndIf
...
function DailyUpdateAttractNewSettlers(WorkshopDataScript:WorkshopRatingKeyword[] ratings, DailyUpdateData updateData)
	...
	if radioRating > 0 && HasKeyword(WorkshopParent.WorkshopType02) == false && updateData.unassignedPopulation < iMaxSurplusNPCs && updateData.totalPopulation < GetMaxWorkshopNPCs()
		...
		if dieRoll <= attractChance
			WorkshopNPCScript newWorkshopActor = WorkshopParent.CreateActorPUBLIC(self)
			...
			if newWorkshopActor.GetValue(WorkshopParent.WorkshopGuardPreference) == 0
				...
				if GetValue(ratings[WorkshopParent.WorkshopRatingBrahmin].resourceValue) == 0.0 && AllowBrahminRecruitment
					...
					if brahminRoll <= WorkshopParent.recruitmentBrahminChance
						;UFO4P 1.0.5. Bug #21002 - Use the new CreateBrahminPUBLIC function here.
						actor newBrahmin = WorkshopParent.CreateBrahminPUBLIC(self)
...
function DailyUpdateProduceResources(WorkshopDataScript:WorkshopRatingKeyword[] ratings, DailyUpdateData updateData, ObjectReference containerRef, bool bRealUpdate)
	...
	WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingMissingSafety].resourceValue, self, missingSafety)
	...
	WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingFoodActual].resourceValue, self, updateData.foodProduction)
	...
	if neededFood > 0 || neededWater > 0
		...
		;UFO4P 1.0.5: Modified traces to prevent them from printing negative values for needed food or needed water (Note: this is a cosmetic fix with no
		;bug tracker entry)
		WorkshopParent.wsTrace(self + "		food=" + Math.Max (0, neededFood) as Int)
		WorkshopParent.wsTrace(self + "		water=" + Math.Max (0, neededWater) as Int)
...
function DailyUpdateConsumeResources(WorkshopDataScript:WorkshopRatingKeyword[] ratings, DailyUpdateData updateData, ObjectReference containerRef, bool bRealUpdate)
	...
	;UFO4P - GENERAL NOTES:
	;
	;In the vanilla approach for calculating actors' happinesses, a loop with a large number of condition checks is run on each actor while various variables
	;are tediously incremented or decremented in every loop cycle. Since this wastes performance when a large number of actors has to be handled, this code
	;haa been replaced by a faster algorithm:
	;
	;(1) The values for Missing Beds, MissingFood, MissingWater, as well as the units of food and water to be removed from the container do not need to be
	;obtained by tedious incrementations but can be calculated in 'one-liners'. They are all handled now after the happiness calculation loop has run.
	;(2) A new approach is used for the actual happiness calculation. This is based on the following thoughts: There are four resources that contribute to
	;actor happiness: food, water, beds and shelter. How many actors can benefit from all four is equal to the smallest one of the four values updateData.
	;shelteredBeds, updateData.availableBeds, updateData.availableFood and updateData.availableWater. The next smallest value minus the smallest value tells
	;how many actors can benefit from the remaining three resources and so forth. In addition, we also need to to deal with the reamaining actors who do not
	;benefit from any resource. Thus, irrespective of the actor number, a loop with a maximum of five cylces is sufficient to do the happiness calculation.
	;
	;To accomplish this, the four resource values are filled in the ResourceCount array and sorted. In order to tell which resource has ended up at which
	;array position after sorting (i.e. to discern which resource we will be dealing with in which loop cycle), the start positions (before sorting) are stored
	;in the ResourcePos array which is then sorted simultaneously with the ResourceCount array. After this is done, the actual resource values are not needed
	;anymore; what we need instead are the numbers of actors to whom the individual resources apply. Thus, we calculate them by difference of the resource
	;values and write them at the corresponding positions of the ResourceCount array. In the loop, we start with the maximum possible happiness (since actors
	;handled in the first step benefit from all resources), multiply this with the actor count (from the ResourceCount array) and add the product to the
	;total happiness. Then, we update the number of remaining actors, check which resource whe did just spend (by comparing the values in the ResourecePos array
	;with the number of the loop cycle), subtract the respective happiness banus from the happiness start value and run the next cycle. The loop runs until
	;we have handled all five actor groups (those with four, three, two, one or no resources available) or until there are no actors left to take care about,
	;whichever happens first (in-game tests have shown that, on average, the calculation only takes 2-3 cycles to complete).
	;
	;Since the setup of the arrays takes some time, there will be little to no performance increase for a small number of actors, but it will be significant
	;when large numbers of actors have to be handled. .
	...
	;WorkshopParent.wsTrace(self + "Start - Actor happiness = " + ActorHappiness + "; Actor count = " + ActorCount)
	;WorkshopParent.wsTrace(self + "Start - Total happiness = " + updateData.totalHappiness)
	...
	WorkshopParent.wsTrace(self + "Total actor happiness, humans only = " + updateData.totalHappiness)

	updateData.totalHappiness = updateData.totalHappiness + ( 50 * updateData.robotPopulation)
	WorkshopParent.wsTrace(self + "Total actor happiness, humans + robots = " + updateData.totalHappiness)	
	
	int missingBeds = Math.Max (0, Actors_Human - updateData.availableBeds) As Int
	WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingMissingBeds].resourceValue, self, missingBeds)
	;WorkshopParent.wsTrace(self + "Missing beds  = " + missingbeds)

	If bRealUpdate
	
		int FoodToRemoveFromContainer = Math.Max (0, Actors_Human - updateData.foodProduction) As Int
		int WaterToRemoveFromContainer = Math.Max (0, Actors_Human - updateData.waterProduction) As Int

		containerRef.RemoveItem(WorkshopParent.WorkshopConsumeFood, FoodToRemoveFromContainer)
		containerRef.RemoveItem(WorkshopParent.WorkshopConsumeWater, WaterToRemoveFromContainer)
		
		int missingFood = Math.Max (0, Actors_Human - updateData.availableFood) As Int
		int missingWater = Math.Max (0, Actors_Human - updateData.availableWater) As Int

		WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingMissingFood].resourceValue, self, missingFood)
		WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingMissingWater].resourceValue, self, missingWater)
		
		;WorkshopParent.wsTrace(self + "Missing food  = " + missingFood)
		;WorkshopParent.wsTrace(self + "Missing water = " + missingWater)

	EndIf

;--------------------------------------------------------------------------------------------------------------------------------
;	UFO4P: Replaced vanilla code:
;--------------------------------------------------------------------------------------------------------------------------------
...
;--------------------------------------------------------------------------------------------------------------------------------
	...
	WorkshopParent.SetResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingHappinessTarget].resourceValue, self, updateData.totalHappiness)
	...
	;	WorkshopParent.wsTrace(self + "	CalculateHappiness: delta happiness (int)=" + deltaHappiness)
		...
		WorkshopParent.ModifyResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingHappiness].resourceValue, self, deltaHappiness)
...
function RepairDamageToResource(ActorValue resourceValue)
	...
	if currentDamage > 0
		...
		if !bHealedActor
			...
			WorkshopParent.ModifyResourceDataPUBLIC(damageRating, self, repairAmount*-1.0)
...
function CheckForAttack(bool bForceAttack = false)
	...
	WorkshopParent.ModifyResourceDataPUBLIC(ratings[WorkshopParent.WorkshopRatingLastAttackDaysSince].resourceValue, self, 1.0)
...
;UFO4P: The vanilla version of the following function had a flaw and was rewritten (the vanilla code is preserved below):
;
;The vanilla function always ran three separate checks on water, food and shelter each. If it was found in the first two checks that the actor had no water
;or no food, his maximum happiness value was capped at maxHappinessNoFood or maxHappinessNoWater, respectively. Since both values are lower than maxHappiness
;NoShelter, the (currentHappiness > maxHappinessNoShelter) condition in the last check is never true when the actor has no food or no water or both. Conse-
;wuently, the last check should be in an 'elseif' branch, so it is not carried out when there is no need to do so. This saves performance as it reduces
;the avarage number of checks per actor significantly (the shelter cap applies in very few cases only).
;
;Since the vanilla presets for maxHappinessNoFood and maxHappinessNoWater are the same, the function could be further optimized by combining the separate
;checks for food and water in one single check. As a consequence however, modification of the presets (e.g. by a mod) would require to rewrite the function
;again for the changed values to work. Therefore, the checks were kept separate, but the check for missing food was put in an elseif branch. This should save
;a fair number of checks when the function runs on a large number of actors (NB: In a realistic overhaul, the happiness cap for missing water would likely be
;lower than that for missing food because humans can survive longer with no food than can without water. Thus, the water cap is the worst case and is there-
;fore checked first.)
...
float function CheckActorHappiness (float currentHappiness, bool bFood, bool bWater, bool bBed, bool bShelter)
	...
	elseIf !bFood && currentHappiness > maxHappinessNoFood
		currentHappiness = maxHappinessNoFood
	elseif !bShelter && currentHappiness > maxHappinessNoShelter
...
;--------------------------------------------------------------------------------------------------------------------------------
;UFO4P: Replaced vanilla function:
;--------------------------------------------------------------------------------------------------------------------------------
